...
| 已过期 | 所有用户可登录但无法访问内容 | 院校级过期优先级最高，所有课程和实训均不可访问 |
...
**账号状态管理**
- 支持禁用/启用账号（学生转学、教师离职等场景）；禁用后账号无法登录，数据保留，账号仍占用配额
...
| 院校总授权到期 | 院校所有用户登录后提示授权已到期，无法访问课程和实训内容；账号数据保留 |
| 单门课程授权到期 | 该课程对新学生不可分配；已分配学生可继续访问已完成内容，但无法解锁新小节；课程列表中标注「已过期」 |
...
- **内容管理**：创建/编辑/删除课程、章节、小节；课程上架/下架（下架后新分配不可见，已分配学生的在读进度不受影响）；已上架课程的内容更新（替换视频、修改富文本、更新附件）实时生效，学生下次访问即看到新内容，已完成的小节状态不受影响
...
| 院校总授权到期 | 学生/教师无法访问课程内容，提示「授权已到期，请联系管理员」 |
| 单门课程授权到期 | 已分配学生可继续访问已完成内容，但无法解锁新小节；院校管理员无法将该课程分配给新学生 |
