...
flowchart TD
    subgraph L["1. 获得账号并登录"]
        L1[院校管理员创建学生账号] --> L2[收到通知（含账号密码）]
        L2 --> L3[访问登录页]
        L3 --> L4{输入凭证}
        L4 -->|错误| L3
        L4 -->|账号已禁用| Lx1["❌ 提示「账号已禁用」"]
        L4 -->|院校已停用/过期| Lx2["❌ 提示「院校已停用/授权到期」"]
        L4 -->|正确且状态正常| L5{首次登录?}
        L5 -->|是| L6[强制修改密码] --> L7
        L5 -->|否| L7[进入课程中心落地页]
        L3 -.->|忘记密码| Lf["联系管理员重置（v2.0: 邮箱自助找回）"]
    end

    subgraph C["2. 课程学习（可与实训交替进行）"]
        L7 --> C1{有已分配课程?}
        C1 -->|否| Cx["空状态页：「暂无课程，请联系老师或管理员」"]
        C1 -->|是| C2[查看课程列表，按岗位路径/能力级别筛选]
        C2 --> C3[进入课程详情页]
        C3 --> C4{当前小节类型}
        C4 -->|视频| C4a["播放视频（断点续播）\n播放至末尾才可完成"]
        C4 -->|富文本| C4b[滚动阅读]
        C4 -->|附件| C4c[下载附件]
        C4a & C4b & C4c --> C5["点击「完成本节」→ 下一节自动解锁"]
        C5 --> C6{还有未完成小节?}
        C6 -->|是| C4
        C6 -->|否| C7["课程完成 ✅"]
        C3 -.->|已完成小节| C3r[可随时回访]
    ...
    subgraph T["3. 工程实训（可与课程交替进行）"]
        C2 -.->|切换模块| T1
        C7 --> T1{有已分配实训包?}
        T1 -->|否| Tx["空状态页：「暂无实训，请联系老师或管理员」"]
        T1 -->|是| T2[选择实训包 → 查看任务列表]
        T2 --> T3[阅读任务说明]
        T3 --> T4[在云端桌面中操作和调试]
        T4 --> T5["提交任务（可添加附件）\n环境快照后台异步生成"]
        T5 --> T6["状态 → 待批改"]
    ...
    subgraph F["4. 查看批改反馈"]
        ...
        F1 --> F2{最新批改结果}
        F2 -->|待批改| F1
        F2 -->|已批改| F3["查看得分（0–100）+ 教师反馈"]
        F2 -->|打回重做| F4[查看打回原因和改进建议]
        F3 --> F5{是否重新提交?}
        F5 -->|否| F6[继续下一任务]
        F5 -->|是| T4
        F4 --> T4
...
flowchart TD
    ...
    subgraph G["4. 批改实训"]
        ...
        G1 --> G2[查看学生提交内容\n（快照/附件）]
        G2 --> G3{批改操作}
        G3 -->|评分| G4["给分（0–100）+ 文字反馈\n→ 已批改"]
        G3 -->|打回重做| G5["填写改进建议\n→ 打回重做"]
        G4 & G5 --> G6[快速切换下一条]
        G6 --> G2
        G2 -.-> Gh[查看同一任务的历史提交与批改]
        G4 -.->|修改已有批改| G4
        G5 -.->|改为评分| G4
    end
...
flowchart TD
    subgraph L["1. 获得账号并登录"]
        L0["企业管理员创建院校管理员账号\n并配置配额/课程授权/实训包授权"] --> L1[收到通知]
        L1 --> L2{登录}
        L2 -->|院校异常| Lx["❌ 无法登录"]
        L2 -->|正常| L3[首次登录 → 修改密码 → 进入管理后台]
    end

    subgraph A["2. 管理账号"]
        L3 --> A1[批量导入学生名单]
        A1 --> A1v{格式校验}
        A1v -->|有错误| A1e[提示错误行号+原因]
        A1v -->|通过| A1c{账号配额检查}
        A1c -->|超额| A1q["阻止：已达上限"]
        A1c -->|未超额| A1s[创建成功，通知学生]
        L3 --> A2[创建教师账号\n绑定课程/实训包/班级]
        L3 --> A3[编辑学生/教师基本信息]
        L3 --> A4[重置学生/教师密码]
        L3 --> A5[禁用/启用账号]
    ...
    subgraph D["3. 分配教学内容"]
        ...
        D4 -->|是| D5[批量分配实训包给学生]
        D0 --> D6[将实训包绑定给教师]
        D2 & D5 -.->|纠错/退课| D9["取消分配 → 释放配额"]
    end

    subgraph V["4. 浏览教学内容"]
        D2 --> V1["以教师视角浏览课程（全部解锁）"]
        V1 --> V2[下载教案与课件]
        D5 --> V3[浏览实训任务说明与参考答案]
    end

    subgraph M["5. 日常数据跟踪"]
        V2 & V3 --> M1["查看配额使用与概览数字\n（学生数/教师数/课程分配数/实训完成率等）"]
        M1 -.->|v2.0| M2[详细学习进度报表]
        M2 -.->|v3.0| M3["导出数据报表（Excel）"]
    end
```
...
flowchart TD
    subgraph C["1. 内容准备"]
        C1[创建课程 → 章节 → 小节] --> C2[上传内容（视频/富文本/附件）]
        C2 --> C3{审核无误?}
        C3 -->|否| C2
        C3 -->|是| C4["上架课程 → 已上架"]
        ...
        C4 -.->|需要撤回| Cdn["下架 → 新分配不可见\n已分配学生不受影响"]
        C4b -.->|需要撤回| Cdnb["下架 → 新分配不可见\n已分配学生不受影响"]
        Cdn -.->|重新上架| C4
        Cdnb -.->|重新上架| C4b
    end
